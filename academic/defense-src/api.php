...
// Isolated academic version: configuration must be provided inside this folder.
// Never implicitly reuse the live site's database configuration.
if (!is_file(__DIR__ . '/config.php')) {
    http_response_code(503);
    echo json_encode(['error' => 'نسخهٔ جداگانهٔ دفاع هنوز به پایگاه‌دادهٔ آزمایشی متصل نشده است.']); exit;
}
...
function generateSecureToken($agency, $role, $name, $salt, $plan = 'Basic') {
    return \Amlak\Defense\Jwt::issue(
        ['sub' => hash('sha256', $agency . '|' . $name), 'a' => $agency, 'r' => $role, 'n' => $name, 'p' => $plan],
        $salt, 'amlak-defense-api-v1', 'amlak-defense-web-v1', 108000
    );
}
...
// HS256 only; decode/verify registered claims before trusting agency or role.
if (is_string($authToken) && $authToken !== '') {
    $dec = \Amlak\Defense\Jwt::verify($authToken, APP_SALT, 'amlak-defense-api-v1', 'amlak-defense-web-v1');
    if ($dec && isset($dec['a'], $dec['r'], $dec['n'], $dec['p'])
        && is_string($dec['a']) && is_string($dec['n']) && is_string($dec['p'])
        && in_array($dec['r'], ['مدیر', 'مشاور'], true) && $dec['a'] === $agencyId) {
        $userRole = $dec['r'];
        $userName = $dec['n'];
        $userPlan = $dec['p'];
        if ($userRole === 'مشاور') {
