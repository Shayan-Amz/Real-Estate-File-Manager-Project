...
// Instrumented copy of a24348e: same provider/model/protocol; timing and size guard only.
...
function stt_curl_run($ch) {
    ...
    $errno     = curl_errno($ch);
    $GLOBALS['evalAsrTrace'][] = ['http' => $httpCode, 'errno' => $errno,
...
function stt_call_hf($bytes, $mime) {
    ...
    if ($r['err'] !== '' || $r['errno'] !== 0) {
        error_log('[thesis-eval/STT] network error');
        return ['ok' => false, 'text' => '', 'provider' => 'hf',
    ...
    error_log('[thesis-eval/STT] HTTP ' . $r['http']);
