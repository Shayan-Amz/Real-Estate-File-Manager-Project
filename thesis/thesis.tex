...
%% نسخه ۲ (بازسازی تمیز؛ همان محتوا و اصلاحیه‌ها با پیش‌درآمد حداقلی) — خروجی: کد LaTeX
...
%% Vazirmatn. بسته‌های لازم: tikz، algorithm، algorithmic، listings، amsmath.
%% =====================================================================
\documentclass[11pt, a4paper]{report}
\usepackage{iftex}
\ifXeTeX\else
  \PackageError{thesis}{Please compile with XeLaTeX (Overleaf: Menu -> Compiler -> XeLaTeX)}{See README.fa.md}%
  \endinput
\fi
\usepackage[top=2.5cm, bottom=2.5cm, left=2.5cm, right=3.5cm]{geometry}
\usepackage{fontspec}

%% FIX (compile): bidi=bidi طبق تجویز بابل برای XeTeX (سریع، مناسب سقف زمانی Overleaf رایگان).
%% V2: فقط بسته‌های لازم؛ amsmath برای ‎\text‎ در فرمول‌ها لازم است و قبل از
%% ذخیره جدول لود می‌شود. float برای ‎\floatname‎ الگوریتم لازم است.
\usepackage{amsmath}
\usepackage{float}
%% V2: ذخیره جدول استاندارد هسته «قبل» از bidi؛ هیچ بسته دیگری جدول را دست نمی‌زند
%% (array/tabularx/booktabs حذف شدند چون هر ۷ جدول فقط با امکانات هسته نوشته شده‌اند).
\makeatletter
\let\amlak@tabular\tabular
\let\amlak@iitabular\@tabular
\let\amlak@endtabular\endtabular
\let\amlak@array\@array
\let\amlak@endarray\endarray
\let\amlak@arraycr\@arraycr
\let\amlak@tabularcr\@tabularcr
\let\amlak@xtabularcr\@xtabularcr
\let\amlak@hline\hline
\makeatother
\usepackage[persian, bidi=bidi, provide=*]{babel}

%% FIX (compile): حذف onchar=ids fonts — تعویض خودکار قلم در مرز حروف با
%% موتور TikZ و محیط لیستینگ تداخل می‌کرد (۱۵۰ خطا، به‌هم‌ریختگی چیدمان و
%% لاتین ماندن شماره‌ها). چون واژه لاتین وسط متن فارسی نداریم، این گزینه لازم نیست.
\babelprovide[import, mapdigits]{persian}
\babelprovide[import]{english}

%% FIX (fonts): انتخاب خودکار قلم در چند سطح پشتیبان (وزیرمتن داخل پروژه،
%% سپس قلم سیستم، سپس تک‌لایو) تا متن فارسی در هیچ محیطی سفید نماند.
\tracinglostchars=2 % هشدار در لاگ اگر حرفی در قلم نباشد (بدون توقف کامپایل)
\IfFileExists{fonts/Vazirmatn-Regular.ttf}{%
  \typeout{[thesis] fonts: using project fonts from fonts/}%
  ...
  \babelfont[persian]{rm}{Vazirmatn}[Path = fonts/, Extension = .ttf, Ligatures = TeX,
    UprightFont = *-Regular, BoldFont = *-Bold,
    ItalicFont = *-Regular, BoldItalicFont = *-Bold]%
}{%
\IfFileExists{thesis/fonts/Vazirmatn-Regular.ttf}{%
  \typeout{[thesis] fonts: using project fonts from thesis/fonts/}%
...
\IfFontExistsTF{Vazirmatn}{%
  \typeout{[thesis] fonts: using system Vazirmatn}%
...
\IfFontExistsTF{Noto Sans Arabic}{%
  \typeout{[thesis] fonts: using system Noto fonts}%
  \babelfont{rm}{Noto Sans}%
  \babelfont[persian]{rm}{Noto Sans Arabic}%
}{%
  \typeout{[thesis] fonts: using TeX Live Amiri fallback}%
...
\usepackage{titlesec}
\usepackage{tocloft}
%% V2: amsmath/float به قبل از babel منتقل شدند؛ array/tabularx/booktabs حذف شدند
%% (جدول‌ها و فرمول‌ها فقط با هسته لاتک + amsmath کار می‌کنند).
...
%% FIX (compile): hyperfootnotes=false برای جلوگیری از حلقه بی‌نهایت هایپررف با پانویس فارسی؛
%% hypertexnames=false برای یکتایی مقصد لینک‌ها با وجود سه‌بار شماره‌گذاری مجدد صفحات.
\usepackage[colorlinks=true, linkcolor=black, citecolor=black, urlcolor=blue,
  hyperfootnotes=false, hypertexnames=false]{hyperref}
...
%% V2: بیمه ــ اگر جایی نسخه bidi جدول اجرا شد، به‌جای خطا نادیده گرفته شود.
\providecommand*\UseMathForPositioningText{}
%% V2: بازیابی جدول استاندارد در هوک آغاز سند (لایه اول؛ لایه دوم ابتدای بدنه است).
\makeatletter
\AtBeginDocument{%
  \let\tabular\amlak@tabular
  \let\@tabular\amlak@iitabular
  \let\endtabular\amlak@endtabular
  \let\@array\amlak@array
  \let\endarray\amlak@endarray
  \let\@arraycr\amlak@arraycr
  \let\@tabularcr\amlak@tabularcr
  \let\@xtabularcr\amlak@xtabularcr
  \let\hline\amlak@hline
}
\makeatother

\begin{document}
%% V2: بازیابی نهایی جدول استاندارد؛ بعد از همه هوک‌ها اجرا می‌شود.
%% ستون‌ها چپ‌به‌راست می‌شوند ولی محتوای فارسی سلول‌ها کاملاً درست است.
\makeatletter
\let\tabular\amlak@tabular
\let\@tabular\amlak@iitabular
\let\endtabular\amlak@endtabular
...
%% V2 (rebuild): نمودار توالی ورود و صدور توکن (تکمیل بصری بند ۴-۲).
شکل \ref{fig:login_seq} توالی کامل ورود کاربر، صدور توکن امضاشده و استفاده از آن در درخواست‌های بعدی را خلاصه می‌کند.

\begin{figure}[htbp]
  \centering
  \begin{tikzpicture}[x=1cm, y=1cm]
    \tikzset{sq/.style={draw, thick, rounded corners=2pt, align=center, font=\small, minimum width=3.4cm, minimum height=0.8cm}}
    \node[sq] (u) at (11.5,0) {مرورگر کاربر};
    \node[sq] (s) at (7.3,0) {سرور متمرکز};
    \node[sq] (d) at (3.1,0) {پایگاه داده};
    \draw[thick, dashed] (11.5,-0.5) -- (11.5,-6.5);
    \draw[thick, dashed] (7.3,-0.5) -- (7.3,-6.5);
    \draw[thick, dashed] (3.1,-0.5) -- (3.1,-6.5);
    \draw[amlakarrow] (11.5,-1.3) -- node[amlaknote, above] {نام کاربری و گذرواژه} (7.3,-1.3);
    \draw[amlakarrow] (7.3,-2.2) -- node[amlaknote, above] {خواندن گذرواژه درهم‌سازی‌شده} (3.1,-2.2);
    \draw[amlakarrow] (3.1,-3.0) -- node[amlaknote, above] {رکورد عضو} (7.3,-3.0);
    \draw[amlakarrow] (7.3,-3.9) -- node[amlaknote, above] {توکن امضاشده} (11.5,-3.9);
    \draw[amlakarrow] (11.5,-4.8) -- node[amlaknote, above] {درخواست با توکن} (7.3,-4.8);
    \draw[amlakarrow] (7.3,-5.7) -- node[amlaknote, above] {پاسخ جیسون} (11.5,-5.7);
    \node[font=\scriptsize] at (7.3,-7.0) {زمان از بالا به پایین پیش می‌رود};
  \end{tikzpicture}
  \caption{نمودار توالی ورود کاربر، صدور توکن و فراخوانی امن}
  \label{fig:login_seq}
\end{figure}
...
\begin{otherlanguage}{english}
%% FIX (compile): listings زبان JavaScript ندارد؛ Java جایگزین شد.
\begin{lstlisting}[language=Java]
...
\label{tab:perf_ai}
\renewcommand{\arraystretch}{1.5}
%% FIX: راست‌چین برای خوانایی فارسی جدول.
