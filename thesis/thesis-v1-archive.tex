...
%% FIX (compile): موتور XeLaTeX با بسته bidi؛ سریع و سازگار با محدودیت زمانی
%% نسخه رایگان Overleaf (لوالاتک برای این سند بیش از حد کند است).
\usepackage{iftex}
\ifXeTeX\else
...
%% FIX (compile): پکیج‌های جدول قبل از bidi لود می‌شوند تا نسخه استاندارد
%% و مدرن جدول (post-array) ذخيره و از بازنويسی خراب bidi در امان بماند.
\usepackage{amsmath}
\usepackage{booktabs}
\usepackage{array}
\usepackage{tabularx}
\usepackage{float}
%% FIX (compile): ذخيره جدول استاندارد؛ اين نقطه بعد از array و قبل از bidi است.
\makeatletter
...
\usepackage{titlesec}
\usepackage{tocloft}
%% NOTE: amsmath/booktabs/array/tabularx/float به قبل از babel منتقل شدند
%% (ذخيره جدول سالم بايد بعد از array و قبل از bidi باشد).
...
%% FIX (compile): بازگردانی جدول استاندارد لاتک؛ بازنویسی bidi از جدول در
%% تک‌لایو ۲۰۲۶ با نسخه جدید آرایه ناسازگار است (خطا در هر tabular).
%% ستون‌ها چپ‌به‌راست می‌شوند ولی محتوای فارسی سلول‌ها کاملاً درست است.
%% بيمه: اگر جايی نسخه bidi اجرا شد، به‌جای خطا ناديده گرفته شود.
...
\begin{document}
%% FIX (compile): بازيابی نهايی جدول استاندارد؛ اين خط‌ها بعد از همه هوک‌های
%% آغاز سند اجرا می‌شوند و بازنويسی خراب bidi را در هر حالتی خنثی می‌کنند.
%% ستون‌ها چپ‌به‌راست می‌شوند ولی محتوای فارسی سلول‌ها کاملاً درست است.
\makeatletter
\let\tabular\amlak@tabular
\let\@tabular\amlak@iitabular
